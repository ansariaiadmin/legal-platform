...
if [[ "NestJS + Next.js + Postgres pgvector" == *"Python"* ]]; then
...
if [[ "NestJS + Next.js + Postgres pgvector" == *"Node"* ]] || [[ "NestJS + Next.js + Postgres pgvector" == *"Next"* ]] || [[ "NestJS + Next.js + Postgres pgvector" == *"Nest"* ]]; then
  ...
  if [ -f docker-compose.yml ]; then
    ...
    for i in {1..30}; do
      ...
      if command -v curl &> /dev/null; then
        if curl -sf http://localhost:8080/api/health یا http://localhost:3001/api/health >/dev/null 2>&1 || curl -sf http://localhost:3000 >/dev/null 2>&1 || curl -sf http://localhost:8000 >/dev/null 2>&1 || curl -sf http://localhost:8080 >/dev/null 2>&1; then
          echo ""
...
echo -e "  آدرس / URL: http://localhost:8080"
echo -e "  ورود / Login: ادمین پس از نصب ساخته می‌شود / npm run db:seed"
echo -e "  سلامت / Health: http://localhost:8080/api/health یا http://localhost:3001/api/health"
echo ""
