...
echo -e "${CYAN}"
cat <<'BANNER'
Legal Platform — v3.1.0 — تاریکی روشن شد — پشتیبانی صفر
دفتر وکالت هوشمند + 6 AI وکیل + SMS نوبت‌دهی + Zarinpal + PWA + E-Signature — تاریکی روشن شد
AI 6 وکیل + SMS نوبت‌دهی + Zarinpal + Email + Telegram + PWA + E-Signature
BANNER
echo -e "${NC}"
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  🧙‍♂️ جادوگر نصب Legal Platform v3.1.0 — تاریکی روشن شد${NC}"
echo -e "${BLUE}  AI 6 وکیل + SMS نوبت‌دهی + Zarinpal + Email + Telegram + PWA + E-Signature${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
echo -e "${YELLOW}سلام! 👋 دفتر وکالت هوشمند + 6 AI وکیل + SMS نوبت‌دهی + Zarinpal + PWA + E-Signature — تاریکی روشن شد${NC}"
...
if [ "legal-platform" = "aark-kernel" ]; then
...
if [ "legal-platform" = "legal-platform" ]; then
...
if [ "legal-platform" = "universal-document-os" ]; then
...
if [ "$TELEGRAM_ENABLED" = "yes" ]; then
  ...
  if [ -n "$TELEGRAM_BOT_TOKEN" ] && command -v curl &> /dev/null; then
    ...
    if [ -n "$TELEGRAM_CHAT_ID" ]; then
      curl -sf -X POST -H "Content-Type: application/json" -d "{\"chat_id\":\"$TELEGRAM_CHAT_ID\",\"text\":\"🧪 تست Legal Platform v3.1.0 — تاریکی روشن شد — $(date)\"}" https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/sendMessage -o /dev/null 2>&1 && ok "Telegram پیام تست فرستاده شد — چک کن — تاریکی روشن شد" || err "Telegram پیام fail — Chat ID چک کن — ربات استارت کردی؟ — تاریکی روشن شد"
    fi
  fi
...
if [ "$SKIP_ENV" = "true" ]; then
  ...
  cat > .env <<EOF
# Legal Platform — .env — v3.1.0 — تاریکی روشن شد — $(date)
...
# AI — 6 وکیل — با هزینه — تاریکی روشن شد
AI_PROVIDER=${AI_PROVIDER}
OPENAI_API_KEY=${AI_KEY}
ANTHROPIC_API_KEY=${AI_KEY}

# Payment — Zarinpal — با هزینه — تاریکی روشن شد
PAYMENT_PROVIDER=${PAYMENT}
ZARINPAL_MERCHANT_ID=${ZARINPAL_KEY}

# PWA + E-Signature — همیشه روشن — سقف — تاریکی روشن شد
PWA_ENABLED=true
...
ADMIN_EMAIL=admin@legal-platform.dev
...
echo ""
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}  🎉 جادو تمام! Legal Platform آماده — تاریکی روشن شد! 🎉${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
echo -e "${BOLD}${BLUE}📍 دسترسی:${NC}"
echo -e "${GREEN}  🌐 http://localhost:3000 — دفتر وکالت هوشمند + 6 AI وکیل + SMS نوبت‌دهی + Zarinpal + PWA + E-Signature — تاریکی روشن شد${NC}"
