...
curl -X POST http://localhost:3000/api/auth/login \
  ...
  -d '{"email":"admin@legal-platform.dev","password":"Admin@123"}'
```
...
{
  "token": "jwt-token-...",
  "user": { "email": "admin@legal-platform.dev", "role": "admin" }
}
```
...
```json
{
  ...
  "providers": {
    ...
    "telegram": { "status": "ok", "bot": "@legal-platform_notif_bot" }
  },
  "notifications": {
...
1. تلگرام → @BotFather → /newbot → اسم → یوزرنیم (مثل legal-platform_notif_bot)
