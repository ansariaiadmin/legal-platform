...
## کانفیگِ صفرمهندسی + آشپزخانهٔ زنده (ADR-014)

- تب‌های داشبورد: **اتصال مغز** (محلی/ابری + تست صادق)، **ناوگان** (کارت
  شخصی هر ایجنت + مغزِ سپرده‌شده یا «قرض از لیدر»)، **چت با لیدر** (حتی برای
  کانفیگ: «به مدل محلی وصل شو …» → کارت پیشنهاد → «بله»)، **فایل‌ها**،
  **آشپزخانهٔ زنده** (گراف زندهٔ SSE — بسته‌های رنگی بین لیدر و کارشناس‌ها
  می‌پرند تا تویِ چشم کار را دنبال کنی).
- تنها دکمه‌ای که وجود ندارد: «دیتای محرمانه به ابر» — آن سوییچ عمداً حذف
  است؛ قانون حریم‌داده در روتر حک شده (ADR-004).
- `DEV_DASHBOARD_TOKEN` فقط بیرون از production درها را برای سندباکس باز
  می‌کند؛ در production بی‌اثر است.
