...
- [ ] #6 فاکتور ایمیل به‌عنوان مرز اصلی + step-up-auth برای `ops/backup`، پیکربندی مغز، ضرب‌گلولهٔ اسرار
- [ ] #10 سیاست خروج به‌ازای فایل privileged (`local-only` پیش‌فرض، نقض با step-up + audit)
...
- [ ] #5g پیش‌فرض اسکن نگهبان در پروداکشن + سیاست بودجهٔ AI هارد‌استوپ/تنزل خودکار
