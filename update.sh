...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  Legal Platform — سیستم حقوقی برای وکلای ایرانی - آپدیت خودکار${NC}"
...
if command -v curl &> /dev/null; then
  if curl -sf http://localhost:8080/api/health یا http://localhost:3001/api/health >/dev/null 2>&1 || curl -sf http://localhost:3000 >/dev/null 2>&1 || curl -sf http://localhost:8000 >/dev/null 2>&1 || curl -sf http://localhost:8080 >/dev/null 2>&1; then
    ...
    curl -sf http://localhost:8080/api/health یا http://localhost:3001/api/health 2>&1 | head -n 5 || true
  else
...
echo -e "${GREEN}========================================${NC}"
echo "آدرس / URL: http://localhost:8080"
